...
/**
 * REST controller for managing {@link co.edu.ucentral.domain.Consulta}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ConsultaResource {
    ...
    public List<Consulta> getAllConsultas(@RequestParam(required = false) String filter) {
        if ("historiausuario-is-null".equals(filter)) {
            log.debug("REST request to get all Consultas where historiaUsuario is null");
            return StreamSupport
                .stream(consultaRepository.findAll().spliterator(), false)
                .filter(consulta -> consulta.getHistoriaUsuario() == null)
